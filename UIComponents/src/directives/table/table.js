...
class TableDirective {
    ...
    link(scope, element, attrs) {
        ...
        let setColWidthByIndex = function (index, increase) {
            ...
            let columnCount = scope.cols.length;
            ...
            nextBodyCol.css('width', index + 1 !== columnCount - 1 ? nextBodyColWidth : (nextBodyColWidth - vScrollWidth));
        ...
        let updateColumnsWidth = () => {
            ...
            invisibleCols.each(function (i, item) {
                ...
                $(item).css(
                    'width', i !== invisibleCols.length - 1 ? width : (width - vScrollWidth)
                );
        ...
        angular.extend(
            scope,
            {
                ...
                onSort(order, column, index) {
                    ...
                    col.sortOrder = col.sortOrder ? (col.sortOrder === 'asc' ? 'desc' : 'asc') : order;
                    scope.rows.sort(getCompareFun(col.sortOrder));
                },
                doFilter(val) {
                    let col = scope.cols[scope.colIndex];
                    let filterValue = val;
