...
export default class DirectedGraphDirective {
    ...
    link(scope, element, attrs) {
        ...
        scope.option.nodes = (scope.option || {}).data || (scope.option || {}).nodes || [];
    ...
    init(scope, element, attrs) {
        ...
        let symbolSize = scope.symbolSize || 20;
            ...
            series: [
                {
                    type: 'graph',
                    layout: 'none',
                    symbolSize: symbolSize, // default 50
                    // symbolOffset: [0, '50%'],
                    roam: false,
                    label: {
                        normal: {
                    ...
                    // left: 0, // symbolSize * 2,
                    // right: 0, // symbolSize * 2,
                    // top: 0, // symbolSize * 2,
                    // bottom: 0, // symbolSize * 2,
                    edgeSymbol: attrs.withArrow === 'false' ? ['circle'] : ['circle', 'arrow'],
    ...
    drawChart(nodes, option, scope) {
        ...
        if (nodes.length) {
            console.log('draw :)', scope.option);
            scope.chartInst.setOption(scope.option);
            angular.element(scope.canvasBox).find('canvas').css({
                left: 0,
                top: 0
            });
        }
        else {
            angular.element(scope.canvasBox).html([
    ...
    generateOption(nodes, scope) {
        ...
        let symbolSize = option.series && option.series[0].symbolSize || 50;
        ...
        if (option.series[0]) {
            option.series[0].data = nodes.map(node => {
                ...
                // adjust boundary node position
                if (node.x <= symbolSize) {
                    node.x += symbolSize;
                    console.log('add x :)', node.x, node);
                }
                if (node.x >= scope.width - symbolSize) {
                    node.x -= symbolSize;
                    console.log('minus x :)', node.x, node);
                }

                if (node.y <= symbolSize) {
                    node.y += symbolSize;
                    console.log('add y:)', node.y, node);
                }
                if (node.y >= scope.height - symbolSize) {
                    node.y -= symbolSize;
                    console.log('minus y:)', node.y, node);
                }
    ...
    drag(el, target, callback) {
        let mousedownHandle = function (evt) {
            ...
            let prePos = angular.extend({}, startPos);
            let mousemoveHandle = function (evt) {
                if (isMousedown) {
                    let offset = angular.element(target).offset();
    ...
    clickNodeHandle(scope, evt) {
        if (evt.dataType === 'node') {
            scope.onClick && scope.onClick.call(scope.context || this, evt.data, scope.option);
            ...
            scope.parentsQueue.push(angular.extend([], scope.nodes));
            scope.pageIndex = 1;
            scope.nodes = angular.extend([], evt.data.children);
