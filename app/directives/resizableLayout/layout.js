...
class LayoutController {
    constructor($scope, $element, $attrs) {
        this.getResizable = () => $attrs.resizable === 'true';
    }
}

LayoutController.$inject = ['$scope', '$element', '$attrs'];
...
class LayoutTopDirective {
    ...
    link(scope, element, attrs, layoutCtrl) {
        ...
        if (resizable) {
            angular.element(this.$compile('<layout-spliter direction="h" target="top"/>')(scope)).insertAfter(angular.element(element));
        }
    }
...
class LayoutLeftDirective {
    ...
    link(scope, element, attrs, layoutCtrl) {
        ...
        if (attrs.width) {
            angular.element(element).css('width', attrs.width);
        }

        let resizable = angular.isUndefined(attrs.resizable) ? layoutCtrl.getResizable() : attrs.resizable;
        if (resizable) {
            angular.element(this.$compile('<layout-spliter direction="v" target="left"/>')(scope))
                .insertAfter(angular.element(element));
        }
    }
...
class LayoutBottomDirective {
    ...
    link(scope, element, attrs, layoutCtrl) {
        ...
        if (attrs.height) {
            // angular.element(element).height(attrs.height);
            angular.element(element).css('height', attrs.height);
        }

        let resizable = angular.isUndefined(attrs.resizable) ? layoutCtrl.getResizable() : attrs.resizable;
        if (resizable) {
            angular.element(this.$compile('<layout-spliter direction="h" target="bottom"/>')(scope))
                .insertBefore(angular.element(element));
        }
    }
...
class LayoutSpliterDirective {
    ...
    link(scope, element, attrs) {
        ...
        let mouseupHandle = function (evt) {
            isMousedown = false;
            angular.element(document).off('mousemove', mousemoveHandle);
            angular.element(document).off('mouseup', mouseupHandle);
        };
        ...
        scope.mousedownHandle = function (evt) {
            ...
            angular.element(document).on('mousemove', mousemoveHandle);
            angular.element(document).on('mouseup', mouseupHandle);
        };
    }
}
